...
public class FourStageEmptyQuery implements NexmarkQuery {
    private final LatencyCount<String, Event> latCount;

    public FourStageEmptyQuery(final String statsDir) {
        final String tag = "fourStageEmpty_sink_ets";
    ...
    public StreamsBuilder getStreamBuilder(final String bootstrapServer, final String serde, final String configFile) throws IOException {
        ...
        final String mid3Tp = prop.getProperty("mid3.name");
        final int numParMid3 = Integer.parseInt(prop.getProperty("mid3.numPar"));
        final NewTopic mid3 = new NewTopic(mid3Tp, numParMid3, REPLICATION_FACTOR);
        ...
        builder.stream("nexmark_src", Consumed.with(Serdes.String(), eSerde)
            ...
            .repartition(Repartitioned.with(Serdes.String(), eSerde)
                .withName(mid3Tp).withNumberOfPartitions(numParMid3))
    ...
    public Properties getExactlyOnceProperties(final String bootstrapServer, final int duration,
        ...
        props.put(StreamsConfig.APPLICATION_ID_CONFIG, "four-empty");
        props.put(StreamsConfig.CLIENT_ID_CONFIG, "four-empty-client");
        return props;
    }
